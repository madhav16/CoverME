...
<?php
    ...
    if(isset($_GET['phone_name']) && $_GET['phone_name']!='' && isset($_GET['skin_name']) && $_GET['skin_name']!='')
    {
        ...
        $sql_phone_name="select * from test_skin where phone_name='$s_phone_name' AND s_name='$s_skin_name' order by id asc";
        $res_phone_name=mysqli_query($con,$sql_phone_name);
       // echo $sql_phone_name;
        while($row=mysqli_fetch_array($res_phone_name)){
            $skin_name=$row['s_name'];
            $cat=$row['cat'];
            $mrp=$row['mrp'];
            $phone_name=$row['phone_name'];
            $des_skin=$row['des_skin'];
            $data=$row['img'];
        }
...
<form action="add_to_cart_php.php" method="POST" enctype="multipart/form-data">
    ...
    <div class="hor">
        ...
        <!-- <img style="width:25%; border-radius:15px;" name="file" src="data:image/jpeg;base64,'.base64_encode(<?php //$data ?>).'"> -->
        <?php echo '<img style="  margin-bottom:3rem; border:10px #ffbb00 solid; width:25%; border-radius:20px;" name="file" src="data:image/jpeg;base64,'.base64_encode($data).'">';?>
        <div class="farm"><h4>
            Skin Name:<input type="text" name="p_name" value="<?php echo $skin_name; ?>" readonly >
        </h4>
        <h4>
        ...
        <h4>
            About Skin:<br><input type="textarea" class="abs" name="about_p" style="margin-top: .3rem;" value="<?php echo $des_skin; ?>" placeholder="describe your product..." readonly >
        </h4>
        <h4>
